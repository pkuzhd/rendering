...
public:
    ...
    my_index_t element_num() const override{
        return mHeader.mElementList.size();
    }
    ...
    const my_value_t &get_value_list(const my_index_t &line_index , const my_index_t &property_index) const override{
        return mPlyValueList[line_index][property_index];
    }
    ...
    const my_byte_t &get_byte_list(const my_index_t &line_index , const my_index_t &property_index) const override{
        return mPlyByteList[line_index][property_index];
    }
    const std::string &get_comment_type() const override{
        return mComment.mName;
    }
    const std::string &get_comment_address()const override{
        return mComment.address;
    }
private:
	...
	void read_header () {
		...
		{
			r1x.insert (pair < string ,int64_t > ("float32" ,PLYREADER_PROPERY_TYPE_VAL32)) ;
			r1x.insert (pair < string ,int64_t > ("float64" ,PLYREADER_PROPERY_TYPE_VAL64)) ;
		...
		map<string ,int64_t> r2x ;
		{
			...
			r2x.insert (pair < string ,int64_t > ("uint8" ,PLYREADER_PROPERY_TYPE_BYTE)) ;
            ...
            r2x.insert (pair < string ,int64_t > ("float32" ,PLYREADER_PROPERY_TYPE_VAL32)) ;
		}
		...
		while (mPlyFile.good()) 
		{
			...
			{
                mComment.mName = header[1];
                mComment.address = header[2];
			}
